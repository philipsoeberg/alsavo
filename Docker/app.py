...
class mqtt_stub:
    ...
    def connect(self, broker_server, broker_server_port, timeout):
        print(
            "Connecting to test stub "
            + broker_server
            + ":"
            + str(broker_server_port)
            + " | "
            + str(timeout)
        )
            ...
            for heatpump in config["heatpumps"]:
                ...
                for line in response.split("\n"):
                    ...
                    if m:
                        ...
                        config_topics = config_loader.find_topics(
                            heatpump["topics"], inputType, inputIndex
                        )
                        ...
                        for config_topic in config_topics:
                            ...
                            out = json.dumps(
                                {"time": round(time.time() * 1000), "value": inputValue}
                            )
                            print(
                            ...
                            client.publish(config_topic["topic"], out, retain=True)

        except Exception as inst:
            print(
                datetime.now()
                .strftime("%Y-%m-%d, %H:%M:%S")
                .join(traceback.TracebackException.from_exception(inst).format())
            )
